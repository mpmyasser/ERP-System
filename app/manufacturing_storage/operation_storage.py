...
def update_cut_item_dispatch(
    ...
    with get_connection() as connection:
        ...
        if not normalized_factory_code or not normalized_dispatch_date:
            status_value = "داخل القص"
        else:
...
def bulk_dispatch_cut_items() -> int:
    with get_connection() as connection:
...
def purge_reference_items() -> dict[str, Any]:
    with get_connection() as connection:
        counts_row = connection.execute(
            """
        ...
        connection.execute("DELETE FROM operation_products")
        connection.execute("UPDATE operation_cut_items SET product_id = NULL")
        connection.execute(
            """
            DELETE FROM sqlite_sequence
            WHERE name = 'operation_products'
            """
        )
        connection.commit()
        connection.execute("PRAGMA foreign_keys = ON")
    ...
    return {
        "deleted_products": int(counts_row["total_products"] or 0) if counts_row else 0,
    }
