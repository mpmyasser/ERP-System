...
class DBManager:
    ...
    def bulk_update_salaries(self, updates, effective_date=None):
        """
        Update multiple employee salaries at once.
        updates: list of dicts [{'employee_id': 1, 'basic_salary': 5000}, ...]
        effective_date: optional datetime for the change to take effect
        """
        session = self.get_session()
        try:
            for update in updates:
                emp_id = update.get('employee_id')
                new_salary = update.get('basic_salary')
                if emp_id and new_salary is not None:
                    emp = session.query(Employee).filter_by(id=emp_id).first()
                    if emp:
                        new_salary_val = float(new_salary)
                        eff_dt = None
                        if effective_date:
                            eff_dt = effective_date
                            if isinstance(eff_dt, date) and not isinstance(eff_dt, datetime):
                                eff_dt = datetime.combine(eff_dt, datetime.min.time())
                            # Pass effective_date to listener via temporary attribute
                            emp._effective_date = eff_dt

                        emp._change_reason = "تعديل جماعي للمرتبات"

                        current_salary = float(emp.basic_salary or 0)
                        if current_salary == new_salary_val and eff_dt:
                            # If salary already equals the new value, force history creation
                            # based on the effective date to avoid silent no-ops.
                            prev_q = session.query(SalaryHistory).filter(
                                SalaryHistory.employee_id == emp.id
                            )
                            prev_q = prev_q.filter(SalaryHistory.effective_date <= eff_dt)
                            prev = prev_q.order_by(
                                SalaryHistory.effective_date.desc(),
                                SalaryHistory.change_date.desc()
                            ).first()

                            if prev and float(prev.new_salary) != new_salary_val:
                                history_entry = SalaryHistory(
                                    employee=emp,
                                    old_salary=float(prev.new_salary),
                                    new_salary=new_salary_val,
                                    salary_change=new_salary_val - float(prev.new_salary),
                                    change_date=datetime.now(),
                                    effective_date=eff_dt,
                                    reason=getattr(emp, '_change_reason', "تعديل جماعي للمرتبات")
                                )
                                session.add(history_entry)
                                emp.salary_updated_at = datetime.now()
                        else:
                            emp.basic_salary = new_salary_val
            session.commit()
            return True
        except Exception as e:
            session.rollback()
