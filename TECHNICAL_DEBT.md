...
| Field | Value |
|-------|-------|
| **Status** | ✅ Fixed (تحقَّقت مباشرة من الكود بتاريخ 2026-08-03: الإصلاح موجود وسليم، فقط تسمية الحالة في هذا الملف انحرفت بسبب تعارض دمج سابق) |
| **Severity** | Low |
...
| Field | Value |
|-------|-------|
| **Status** | ✅ Fixed (2026-08-03) — نُفِّذت بواسطة عضو آخر في الفريق (يبدو أنه "Antigravity")، وأصلحتُ أنا خللًا ناتجًا عن تعارض دمج ملفات تسبَّب في تعطيل الميزة بالكامل (انظر قسم سجل النشاط بتاريخ 2026-08-03) |
...
**التحقق النهائي من الإصلاح (2026-08-03):** الحل المُنفَّذ فعليًا: موديل `BulkSalaryUpdateRequest`
جديد (`idempotency_key` بقيد `unique=True`)، الواجهة تُولِّد المفتاح عبر `crypto.randomUUID()`
وتُرسله مع كل طلب، والخادم يحاول إدراج المفتاح ضمن نفس معاملة تحديث الرواتب — فشل الإدراج
(`IntegrityError`) يعني طلبًا مكرَّرًا فيُرجَع `duplicate: true` بدون تكرار التغيير. **اختبرت
السيناريو بالضبط الذي وصفته TD-006** (طلبان بنفس مفتاح Idempotency، جلستا `Flask test_client`
منفصلتان): الطلب الأول أنشأ سجل `SalaryHistory` واحدًا فقط (`تعديل جماعي للمرتبات`)، والطلب الثاني
المكرر **لم يُنشئ أي سجل إضافي** (تحققت بعدّ السجلات قبل وبعد كل طلب على حدة، وليس فقط في النهاية).
هذا يُغلق TD-006 نهائيًا بدليل تنفيذ مباشر وليس بالثقة في وجود الميزة فقط.
...
| 2026-08-06 | إضافة **TD-009**: توثيق فشل سابق موجود (5/7) في `test_ui_integration_bonus.py` (سكريبت UI مستقل خارج `tests/`)، اكتُشف أثناء التحقق الآلي بعد دفعة P4-L05. أثبتت بالـ`git stash` أن الفشل لا علاقة له بتعديل تنظيف الاستيرادات. لم يُصلَح (خارج نطاق المهمة). |
...
## TD-009 — `test_ui_integration_bonus.py` Fails 5/7 Tests (Pre-existing, Unrelated to Any Refactor)

| Field | Value |
|-------|-------|
| **Status** | ⛔ Open (not fixed — مجرد توثيق) |
| **Severity** | Low (Testing Infrastructure / not part of live app) |
| **Category** | Test Correctness |
| **Discovered during** | التحقق الآلي بعد دفعة P4-L05 (تنظيف استيرادات غير مستخدمة في `utils/helpers.py` و`test_ui_integration_bonus.py`) |
| **Affected files** | `test_ui_integration_bonus.py` (سكريبت اختبارات UI مستقل، خارج `tests/`) |

**التحقق (حقائق مباشرة فقط):**

1. شغَّلت `python test_ui_integration_bonus.py` على الكود **بعد** تعديلي (حذف `import os`) → `Passed: 2/7`,
   `5 TEST(S) FAILED` (TEST 1, 2, 3, 4, 5 فاشلة؛ TEST 6 ناجحة كـ`302`؛ TEST 7 تحذير لكنها تُرجع `True`).

2. **أثبتت أن الفشل سابق موجود تمامًا ولا علاقة له بتعديلي**: عبر `git stash` (أعاد الملفين لحالتهما الأصلية قبل تعديلي)
   ثم إعادة تشغيل نفس الاختبار → **نفس النتيجة بالضبط**: `Passed: 2/7`, `5 TEST(S) FAILED`. بعدها `git stash pop`
   أعاد تعديلاتي. إذن الفشل لا يرجع إلى حذف `import os` (والملف لا يستخدم `os` في أي سطر من كوده على أي حال).

3. الفشل وظيفي (بيانات/حالة)، وليس `NameError` أو خطأ استيراد: TEST 2 يُرجع `404` على `/bonuses/create`
   (يحتاج غالبًا بيانات Employees في قاعدة البيانات الحقيقية، أو جلسة مصادقة، أو مسار مُسجَّل بشكل مختلف)،
   و TEST 1/4/5 تبحث عن حقول/عناصر HTML محددة لا تظهر في الرندر الفارغ. لا توجد أخطاء استيراد أو `NameError`.

4. هذا الملف **خارج مجلد `tests/` الرسمي** الذي تُشغَّل عليه `pytest tests/` (والذي يُعطي `37 passed`) —
   أي أنه **غير مُدرَج في دورة الاختبار الرسمية** الحالية. لا يُكتشَف فشله إلا بتشغيله يدويًا.

**لماذا تم التأجيل:** الاكتشاف حدث أثناء التحقق من مهمة نظافة استيرادات (P4-L05)، ولا علاقة له بفشل وظائف Bonus UI.
إصلاحه يتطلب تحقيقًا منفصلًا (هل المسارات تتطلب بيانات/مصادقة؟ هل حقول النموذج تغيَّرت؟ هل الاختبار نفسه مهجور؟)
— خارج نطاق مهمة تنظيف الاستيرادات.

**Business Impact:** ضئيل — سكريبت اختبار UI مستقل غير مُشغَّل تلقائيًا في دورة الاختبار الرسمية (`tests/`).
قيمته الحالية كشبكة أمان شبه معدومة (5/7 فاشلة) لكنه لا يُسبِّب فشلًا في الإنتاج.

**Estimated effort:** متوسط (ساعة إلى ساعتين) — تشخيص سبب الـ404 على `/bonuses/create` (بيانات؟ مصادقة؟)
ثم إما توفير fixtures للاختبار أو تحديث التوقعات لمطابقة الرندر الحالي، أو توثيقه كـ"مهجور" وحذفه.

**Related issues:** لا يوجد ذكر له في `AUDIT_REPORT.md`/`VERIFIED_AUDIT_REPORT.md`. الملف من نوع اختبارات UI
اليدوية غير الرسمية. مرتبط بالمنطق نفسه مثل `test_user_delete_button.py` و`test_url_for.py` (سكريبتات اختبار
مستقلة في جذر المشروع خارج `tests/`).

**ملاحظة إضافية (تنبيه lint سابق، لا يخصص لها TD منفصل لتفاهته):** أثناء التحقق الآلي لنفس الملف، رصد
`flake8 --select=F` تنبيهًا سابقًا موجودًا تمامًا: `test_ui_integration_bonus.py:72:19: F541 f-string is missing
placeholders` على السطر `print(f"   ✅ Route returns 200 OK")` (حرف `f` زائد بلا placeholders). لا علاقة له بأي
